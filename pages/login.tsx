    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ width:'100%', maxWidth:420 }}>
          ...
          {/* Form card */}
          <div style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:16, padding:32, boxShadow:'0 4px 24px rgba(0,0,0,0.08)' }}>
            ...
            {error && (
              <div style={{ background:T.dangerBg, border:`1px solid #FECACA`, borderRadius:8, padding:'10px 14px', marginBottom:16, fontSize:13, color:T.danger, display:'flex', alignItems:'center', gap:8 }}>
                ⚠️ {error}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ marginBottom:16 }}>
                ...
                <input type="email" value={email} onChange={e=>setEmail(e.target.value)} onFocus={()=>setFocused('email')} onBlur={()=>setFocused(null)} placeholder="you@venusenergyindia.com" autoComplete="email" style={inputStyle(focused==='email')} />
              </div>
              ...
              <div style={{ marginBottom:24 }}>
                ...
                <div style={{ position:'relative' }}>
                  <input type={showPwd?'text':'password'} value={password} onChange={e=>setPassword(e.target.value)} onFocus={()=>setFocused('password')} onBlur={()=>setFocused(null)} placeholder="••••••••" autoComplete="current-password" style={{ ...inputStyle(focused==='password'), paddingRight:44 }} />
                  <button type="button" onClick={()=>setShowPwd(s=>!s)} style={{ position:'absolute', right:12, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', cursor:'pointer', fontSize:14, color:T.textDim }}>
                    {showPwd?'🙈':'👁'}
                  </button>
                </div>
              </div>

              <button type="submit" disabled={busy} style={{ width:'100%', background:busy?'#5EEAD4':T.primary, color:'#fff', border:'none', borderRadius:9, padding:'11px 0', fontSize:14, fontWeight:600, cursor:busy?'not-allowed':'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8, boxShadow:busy?'none':`0 4px 14px ${T.primary}40`, transition:'background 0.15s' }}>
