  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      const addFooter = (page: number) => {
        ...
        pdf.text(`PM: ${pmName}`, margin, footerY + 5);
        // Center: page number
        pdf.setFont('helvetica', 'normal');
        pdf.text(`Page ${page}`, pageWidth / 2, footerY + 5, { align: 'center' });
        // Right: ஐDataOne — use Unicode Tamil char
        const brandText = `Generated by ஐDataOne — www.idataone.com`;
        const tw = pdf.getTextWidth(brandText);
        pdf.text(brandText, pageWidth - margin - tw, footerY + 5);
        // Reset text color
