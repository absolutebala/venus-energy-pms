...
export default function PMDetailPage() {
  ...
  // Filtered activities by date preset
  const filteredActivities = React.useMemo(() => {
    ...
    return activities.filter((a:any) => {
      ...
      if (activityFilter === 'today') return d >= today;
      if (activityFilter === 'week')  return d >= startOfWeek;
      if (activityFilter === 'month') return d >= startOfMonth;
      return true;
    });
  ...
  const exportActivityPDF = async () => {
    ...
    try {
      const canvas = await html2canvas(activityRef.current, { scale:1.5, useCORS:true, backgroundColor:'#ffffff', logging:false });
      const pdf = new jsPDF({ orientation:'landscape', unit:'mm', format:'a4' });
      const pw = pdf.internal.pageSize.getWidth();
      const ph = pdf.internal.pageSize.getHeight();
      const margin = 8; const footerH = 9; const usableH = ph - margin - footerH;
      const imgW = pw - margin*2;
      const imgH = (canvas.height * imgW) / canvas.width;
      let page = 1; let remaining = imgH; let srcYPx = 0;
      const addActFooter = (p:number) => {
        pdf.setFillColor(13,148,136); pdf.rect(0, ph-footerH, pw, footerH, 'F');
        pdf.setFontSize(7.5); pdf.setTextColor(255,255,255);
        pdf.setFont('helvetica','bold'); pdf.text(`${pmName} — Activities`, margin, ph-footerH+5.5);
      ...
      while (remaining > 0) {
        ...
        if (ctx) { ctx.drawImage(canvas,0,srcYPx,canvas.width,sliceCanvas.height,0,0,canvas.width,sliceCanvas.height); pdf.addImage(sliceCanvas.toDataURL('image/png'),'PNG',margin,margin,imgW,sliceH); }
        addActFooter(page); remaining -= sliceH; srcYPx += sliceCanvas.height;
        if (remaining > 0) { pdf.addPage(); page++; }
      }
      const filterLabel = activityFilter==='today'?'Today':activityFilter==='week'?'This_Week':activityFilter==='month'?'This_Month':'All';
      pdf.save(`Activities_${pmName.replace(/\s+/g,'_')}_${filterLabel}_${new Date().toISOString().slice(0,10)}.pdf`);
        ...
        {[10, 50, 100, 'All'].map(n => (
          <button key={n} onClick={() => { setPerPage(n === 'All' ? 99999 : Number(n)); setPage(1); }}
            style={{ padding: '3px 10px', borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: 'pointer',
              ...
              color: perPage === (n === 'All' ? 99999 : Number(n)) ? '#fff' : T.textMuted }}>
            {n}
          </button>
        ))}
              ...
              <div style={card}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
                  <div style={{ fontSize:14, fontWeight:700, color:T.text }}>
                    📋 {pmName} Activities <span style={{ fontSize:12, color:T.textMuted, fontWeight:400 }}>({filteredActivities.length} of {activities.length})</span>
                  </div>
                  <div style={{ display:'flex', gap:6, alignItems:'center' }}>
                    {(['all','today','week','month'] as const).map(f => (
                      <button key={f} onClick={() => setActivityFilter(f)}
                        style={{ padding:'4px 10px', fontSize:11, fontWeight:600, borderRadius:6, cursor:'pointer',
                          border:`1px solid ${activityFilter===f?T.primary:T.border}`,
                          background:activityFilter===f?T.primary:'#fff',
                          color:activityFilter===f?'#fff':T.textMuted }}>
                        {f==='all'?'All':f==='today'?'Today':f==='week'?'This Week':'This Month'}
                      </button>
                    ))}
                    <button onClick={exportActivityPDF} disabled={exportingActivityPDF || filteredActivities.length===0}
                      style={{ padding:'4px 10px', fontSize:11, fontWeight:600, borderRadius:6, cursor:'pointer',
                        border:'1px solid #BFDBFE', background:'#EFF6FF', color:'#1E40AF',
                        opacity: exportingActivityPDF ? 0.7 : 1 }}>
                      {exportingActivityPDF ? '⏳ Generating…' : '📄 PDF'}
                    </button>
                  </div>
                </div>
                <div ref={activityRef}>
                  {filteredActivities.length === 0 ? (
                    <div style={{ color:T.textMuted, fontSize:13 }}>No activities for the selected period.</div>
                  ) : (
                    <div style={{ overflowX:'auto' as const }}>
                      <table style={{ width:'100%', borderCollapse:'collapse' as const, fontSize:12 }}>
                        <thead>
                        ...
                        <tbody>
                          {filteredActivities.slice(0,200).map((a:any, i:number) => (
                      ...
                      {filteredActivities.length > 200 && <div style={{ marginTop:8, fontSize:12, color:T.textMuted, textAlign:'center' as const }}>Showing latest 200 of {filteredActivities.length} activities</div>}
                    </div>
                  )}
                </div>
              </div>
