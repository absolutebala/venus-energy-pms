...
// TEST ENDPOINT — isolated copy of extract-stn.ts, swapped to gpt-5.6-terra for accuracy comparison.
// Does NOT affect production STN/SRN import (extract-stn.ts is untouched).
// NOTE: gpt-5.6-terra is a newer/reasoning-family model. Two things may differ from gpt-4o's API shape:
//   1. It may require `max_completion_tokens` instead of `max_tokens` (used below).
//   2. `temperature`/`seed` may not be accepted the same way — deliberately OMITTED here for the
//      first test. If the call fails, the returned error text will say exactly which param is rejected.

export const config = { api: { bodyParser: { sizeLimit: '10mb' } } };

// GPT-5.6 Terra pricing (per 1M tokens) — approximate, verify on OpenAI's pricing page before relying on this for real cost tracking.
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const { images, source = 'stn-test', projectId } = req.body as { images: string[]; source?: string; projectId?: string };
...
Return ONLY valid JSON, no markdown:
{
  ...
  "items": [
    ...
    try {
      ...
      return res.status(200).json({ success: true, data: JSON.parse(clean), _rawModel: 'gpt-5.6-terra' });
    } catch {
