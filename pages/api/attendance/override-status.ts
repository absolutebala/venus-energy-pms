...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!targetUserId || !requestDate || !['present', 'absent', 'leave', 'holiday'].includes(newStatus)) {
    return res.status(400).json({ error: 'targetUserId, requestDate, and a valid newStatus (present/absent/leave/holiday) are required' });
  }
